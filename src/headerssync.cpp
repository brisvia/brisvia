    ...
    m_current_height(chain_start->nHeight)
{
    ...
    // Brisvia: on a RandomX chain the header's work can only be credited after validating its RandomX PoW.
    // The transient context starts anchored to the known block (m_chain_start) the peer syncs from.
...
void HeadersSyncState::Finalize()
{
    ...
    m_current_height = 0;
    // Brisvia: free the transient context (and its circular window of block ids) + the pending batch.
    m_randomx_branch_context.reset();
    ClearShrink(m_pending_headers);
    m_pending_pos = 0;
    m_pending_full_headers_message = false;
...
{
    ProcessingResult ret;
    Assume(HasPendingHeaders());
    if (!HasPendingHeaders()) { ret.success = true; return ret; }
    Assume(m_download_state != State::FINAL);
    if (m_download_state == State::FINAL) return ret;
    return ProcessPendingQuantum();
}
...
HeadersSyncState::ProcessingResult HeadersSyncState::ProcessPendingQuantum()
{
    ...
    if (m_download_state == State::PRESYNC) {
        ...
        for (; m_pending_pos < quantum_end; ++m_pending_pos) {
            if (!ValidateAndProcessSingleHeader(m_pending_headers[m_pending_pos])) {
                m_pending_headers.clear(); m_pending_pos = 0;
                ret.status = m_last_randomx_reject; // empty if the failure was not due to RandomX
                Finalize();
                return ret; // success=false
            }
        }
        ...
        for (; m_pending_pos < quantum_end; ++m_pending_pos) {
            if (!ValidateAndStoreRedownloadedHeader(m_pending_headers[m_pending_pos])) {
                m_pending_headers.clear(); m_pending_pos = 0;
                ret.status = m_last_randomx_reject; // empty if the failure was not due to RandomX
                Finalize();
                return ret; // success=false
        ...
        if (m_redownloaded_headers.empty() && m_process_all_remaining_headers) {
            LogDebug(BCLog::NET, "Initial headers sync complete with peer=%d: releasing all at height=%i (redownload phase)\n", m_id, m_redownload_buffer_last_height);
        } else if (m_pending_full_headers_message) {
            ret.request_more = true;
        } else {
            LogDebug(BCLog::NET, "Initial headers sync aborted with peer=%d: incomplete headers message at height=%i (redownload phase)\n", m_id, m_redownload_buffer_last_height);
        }
    ...
    // End of message: clear the pending batch and decide finalization.
    m_pending_headers.clear();
    m_pending_pos = 0;
    if (!(ret.success && ret.request_more)) Finalize();
    return ret;
}
...
void HeadersSyncState::MaybeTransitionToRedownload()
{
...
bool HeadersSyncState::ValidateAndProcessSingleHeader(const CBlockHeader& current)
{
    ...
    // Brisvia: validate the header's RandomX PoW (and advance the context) BEFORE crediting its work. Without this,
    // nBits would produce chainwork before proving the corresponding RandomX -> fake-work DoS in presync.
    if (!BrisviaCheckHeaderRandomX(current)) {
        return false;
    }
...
bool HeadersSyncState::BrisviaCheckHeaderRandomX(const CBlockHeader& header)
{
    ...
    if (st != RandomXHeaderStatus::VALID) {
        ...
        LogDebug(BCLog::NET, "Initial headers sync aborted with peer=%d: RandomX header rejected (status=%d) at height=%i\n",
                 m_id, static_cast<int>(st), static_cast<int>(next_height));
        return false;
    }
    return true;
}
