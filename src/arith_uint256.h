...
protected:
    ...
    uint32_t pn[WIDTH];
public:
    // Brisvia: cross-width access (arith_uint512 <-> arith_uint256, used by ASERT).
    template<unsigned int B> friend class base_uint;

    // Converts from another width by copying the words that fit (truncates if the destination is smaller).
    template<unsigned int B>
    static base_uint from(const base_uint<B>& b)
    {
        const int n = (WIDTH < base_uint<B>::WIDTH) ? WIDTH : base_uint<B>::WIDTH;
        base_uint ret;
        for (int i = 0; i < n; i++)
            ret.pn[i] = b.pn[i];
        return ret;
...
// NOTE: `extern template class base_uint<512>;` is NOT used (unlike <256>). That `extern template`
