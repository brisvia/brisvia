...
public:
    explicit CTestNetParams(const CChainParams::TestNetOptions& opts) {
        ...
        if (opts.brisvia_pow) {
            ...
            pchMessageStart[3] = 0x31; // '1'  ->  "BRT1" (Brisvia testnet), does not collide with any Bitcoin network
            nDefaultPort = 19333;

            consensus.nMinimumChainWork = uint256{}; // set after mining the first blocks (to exercise the low-work F3B)
            consensus.defaultAssumeValid = uint256{};

            // Testnet genesis mined with RandomX (frozen values; regression in test mine_testnet_genesis).
            unsigned char anchor[32]; std::memset(anchor, 0xB2, sizeof(anchor));
            const uint32_t genesisTime = 1751846400, genesisNonce = 0, genesisBits = 0x207fffff;
            genesis = CreateBrisviaGenesisBlock("Brisvia testnet genesis - rx/brva-v1", anchor,
            ...
            assert(consensus.hashGenesisBlock == uint256{"2818dc2dc4b881668c55c35a3925d0ab840f4d900b265c1727f26e3d04dee2a5"});
            assert(genesis.hashMerkleRoot == uint256{"2568cf9e1033415283e52c8ef0f548958edf434f428496df25085a637033004b"});

            // First testnet: small network. Peers via -addnode=187.77.240.145:19333 (documented). No DNS/fixed seeds yet.
            vFixedSeeds.clear();
            vSeeds.clear();
...
public:
    explicit CRegTestParams(const RegTestOptions& opts)
    {
        ...
        if (opts.brisvia_pow) {
            ...
            // fPowNoRetargeting is already true in regtest: difficulty stays constant (ASERT does not react).

            unsigned char anchor[32];
            std::memset(anchor, 0xAB, sizeof(anchor)); // test anchor (mainnet: hash of the public fair-launch data)
            const uint32_t genesisTime = 1751760000, genesisNonce = 2, genesisBits = 0x207fffff;
            genesis = CreateBrisviaGenesisBlock("Brisvia regtest genesis - rx/brva-v1", anchor,
                                                genesisTime, genesisNonce, genesisBits, 1);
            consensus.hashGenesisBlock = genesis.GetHash();
            consensus.asertAnchorParams = Consensus::Params::ASERTAnchor{
                /*nHeight=*/0, /*nBits=*/genesisBits, /*nPrevBlockTime=*/int64_t(genesisTime) - consensus.nPowTargetSpacing};
...
public:
    CBrisviaTestNetParams() {
        m_chain_type = ChainType::BRISVIA_TESTNET;
        consensus.signet_blocks = false;
        consensus.signet_challenge.clear();
        ...
        consensus.asertAnchorParams = Consensus::Params::ASERTAnchor{
            /*nHeight=*/0, /*nBits=*/genesisBits, /*nPrevBlockTime=*/int64_t(genesisTime) - consensus.nPowTargetSpacing};
        assert(consensus.hashGenesisBlock == uint256{"c5a8d09048ea1957010a555490d9f3ea8b30d30f7432b1d84e5e068c666712d1"});
        assert(genesis.hashMerkleRoot == uint256{"2568cf9e1033415283e52c8ef0f548958edf434f428496df25085a637033004b"});
...
{
    return std::make_unique<const CTestNetParams>(TestNetOptions{});
}
