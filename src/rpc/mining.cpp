...
static bool GenerateBlock(ChainstateManager& chainman, CBlock&& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    ...
    if (chainman.GetConsensus().fPowRandomX) {
        ...
        uint256 brisvia_seed;
        {
            ...
            const int nHeight = (tip ? tip->nHeight : -1) + 1;
            brisvia_seed = BrisviaGetSeedHash(tip, nHeight, chainman.GetConsensus());
        }
        while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max()
               && CheckRandomXHeaderWithResolvedContext(block, block.nBits, brisvia_seed, chainman.GetConsensus()) != PoWCheckResult::VALID
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (consensusParams.fPowRandomX) {
        ...
        brisvia.pushKV("pow_version", 1);
        brisvia.pushKV("randomx_seed_hash", BrisviaGetSeedHash(pindexPrev, nHeight, consensusParams).GetHex());
        brisvia.pushKV("seed_height", BrisviaSeedHeight(nHeight));
        brisvia.pushKV("header_size", 80);
        brisvia.pushKV("nonce_offset", 76);
        brisvia.pushKV("nonce_size", 4);
        result.pushKV("brisvia", brisvia);
    }
