...
void PeerManagerImpl::HandleHeadersSyncResult(Peer& peer, CNode& pfrom,
        HeadersSyncState::ProcessingResult& result, std::vector<CBlockHeader>& headers)
{
...
bool PeerManagerImpl::IsContinuationOfLowWorkHeadersSync(Peer& peer, CNode& pfrom, std::vector<CBlockHeader>& headers)
{
    if (peer.m_headers_sync) {
        ...
        HandleHeadersSyncResult(peer, pfrom, result, headers);
...
bool PeerManagerImpl::DrainPendingHeadersQuantum(CNode& pfrom, Peer& peer)
{
    ...
    {
        LOCK(peer.m_headers_sync_mutex);
        if (!peer.m_headers_sync || !peer.m_headers_sync->HasPendingHeaders()) return false;
        had_work = true;
        auto result = peer.m_headers_sync->ContinuePendingHeaders();
        HandleHeadersSyncResult(peer, pfrom, result, headers);
    ...
    if (reject_status.has_value()) {
        ...
        case RandomXHeaderStatus::BAD_BITS:
        case RandomXHeaderStatus::BAD_PREV_BLOCK:
    ...
    // Accept into the index the headers that became ready (REDOWNLOAD phase). In PRESYNC `headers` stays empty.
    if (!headers.empty()) {
